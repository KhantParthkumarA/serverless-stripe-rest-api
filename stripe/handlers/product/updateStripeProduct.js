...
module.exports.updateStripeProduct = async (event) => {
  console.log('running');
  ...
  try {
    let product = await productService.Get(event.pathParameters?.id)
    if (product.statusCode !== 204) {
    ...
    var params = {
      TableName: "products",
      Item: { ...product.Items[0], id: event.pathParameters?.id, status: request.status }
    };
